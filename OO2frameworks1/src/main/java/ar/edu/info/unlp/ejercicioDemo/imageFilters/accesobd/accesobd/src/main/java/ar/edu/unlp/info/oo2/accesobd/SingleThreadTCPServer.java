...
/*
    ...
    protected final void handleClient(Socket clientSocket) {
        ...
        try (
            ...
            preProcessClient(clientSocket); // hook 
            
            while ((inputLine = in.readLine()) != null) {
                System.out.println("Received message: " + inputLine + " from "
                        + clientSocket.getInetAddress().getHostAddress() + ":" + clientSocket.getPort());
                
                if (inputLine.equalsIgnoreCase(getCloseWord())) { // punto II hook para configurar la palabra de cierre
                    break; // Client requested to close the connection
                }
                handleMessage(inputLine, out);
            }
            
            postProcessClient(clientSocket);// hook 
            
            System.out.println("Connection closed with " + clientSocket.getInetAddress().getHostAddress() + ":"
                    + clientSocket.getPort());
        } catch (IOException e) {
            handleClientCommunicationError(e, clientSocket); // hook
        } finally {
            try {
                clientSocket.close();
            } catch (IOException e) {
